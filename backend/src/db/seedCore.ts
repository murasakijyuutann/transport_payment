...
export const ZONE_PAIR_FARES: Array<{ from: string; to: string; amount: string }> = [
...
/** Idempotent network + fare rules + daily cap (safe for tests and re-seed). */
export async function seedNetworkAndRules(): Promise<{
  adultId: string;
  studentId: string;
  ...
  await seedDailyCap();
  return { adultId, studentId, zoneIds };
}
...
export async function upsertDemoUser(
  ...
  if (existing) {
    ...
    if (!account) throw new Error(`Account missing for ${email}`);
    return { accountId: account.id, created: false };
  }
